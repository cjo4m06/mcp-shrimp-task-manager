...
您可以透过设定 `TEMPLATES_USE` 环境变数来切换系统提示词的语言。预设支援 `en`（英文）和 `zh`（简体中文）。此外，您也可以复制现有的模板目录（例如 `src/prompts/templates_en` 或 `src/prompts/templates_zh`）到 `DATA_DIR` 指定的位置，进行修改后，将 `TEMPLATES_USE` 指向您的客制化模板目录名称，以实现更深度的提示词客制化
...
- **TEMPLATES_USE**：指定提示词使用的模板集。预设为 `en`。目前可用的选项有 `en` 和 `zh`（简体中文）。若要使用自定义模板，请将 `src/prompts/templates_en` 或 `src/prompts/templates_zh` 目录复制到 `DATA_DIR` 指定的位置，重新命名复制的目录（例如，`my_templates`），并将 `TEMPLATES_USE` 设置为新的目录名称（例如，`my_templates`）。
